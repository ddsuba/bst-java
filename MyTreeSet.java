...
*/
    ...
    {
        Stack<TreeNode<E> > path = new Stack<TreeNode<E> >();
        TreeNode<E> rt;
        public MyTreeSetIterator( TreeNode<E> root )
        {
            ...
            path.add(root);
            while(path.peek().getLeft() != null)
            {
                path.add(path.peek().getLeft());
            }
            rt = root;
        ...
        public E next()
        {
            E res = path.peek().getValue();
            if (path.peek() == rt)
            {
                path.clear();
                return res;
            }
            if (path.peek().getRight() == null)
            {
                while(!path.isEmpty())
                {
                    TreeNode<E> nd = path.peek(); path.pop();
                    if (nd == path.peek().getLeft())
                    {
                        break;
                    }
                }
